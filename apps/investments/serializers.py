...
class InvestDetailSerializer(serializers.ModelSerializer):
    ...
    # total_asset 계산식이 반복돼서 refactor 하고 싶었지만, SerializerMethodField는 상속이 불가능
    # 계산식을 market_value_formula로 만들어 이용
    def get_account_total_asset(self, obj):
        total_asset = Account.objects.filter(id=obj.id).values('id').annotate(
            account_total_asset=market_value_formula)[0]['account_total_asset']
        return total_asset
...
market_value_formula = Sum(
    F('investment_account_number__quantity') * F('investment_account_number__investment_stock__current_price'))
